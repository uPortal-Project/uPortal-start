...
test("portlet-list POST confirm disallow duplicate name", async ({
  request,
}) => {
...
test("portlet-list happy path", async ({ request }) => {
  ...
  const portletListName = `portlet-list happy path ${dateString}`;
  ...
  jsonBaseline.push(jsonSpecificList);
  expect(jsonNewBaseline).toEqual(jsonBaseline);

  // login_via_api with a different user
  await loginViaApi(request, "staff", "staff", "Samuel Staff");
